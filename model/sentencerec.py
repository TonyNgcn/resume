...
class SentenceRecModel(object):
    def __init__(self, sentence_len: int = config.SENTENCE_LEN, wordvec_size: int = config.WORDVEC_SIZE,
                 ...
                 study_rate: float = config.SR_STUDY_RATE, model_name: str = "sentencerec.ckpt",
    ...
    def get_model(self):
        ...
        with graph.as_default():
            ...
            bigru = keras.layers.Bidirectional(
                keras.layers.GRU(config.SR_BIGRU_UNITS, return_sequences=False, dropout=config.SR_BIGRU_DROPOUT))(ph_x)
...
defualt_model = SentenceRecModel()
